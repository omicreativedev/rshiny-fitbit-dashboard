...
#' FIXED: Empty plot placeholder using ggplot (no Plotly warnings)
...
server <- function(input, output, session) {
  ...
  observeEvent(input$login_btn, {
    ...
    # Set date range from tokenSheet for this participant
  ...
  server <- function(input, output, session) {
    ...
    # ==================== EXTRACT SPECIFIC DATAFRAMES (pattern from example_charts.Rmd) ====================
    ...
    # ==================== METRIC CARDS (with login guards) ====================
    ...
    output$dynamic_tabs <- renderUI({
      req(auth$logged_in)  # explicit guard
      if (auth$is_admin) {
        tabsetPanel(
    ...
    # 3. Sleep Stage Breakdown (handles multiple stage names)
